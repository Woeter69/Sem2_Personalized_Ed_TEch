...
def get_personalized_recommendations(user_profile, weak_chapters, favorite_teachers, learning_speed):
    if not client:
    ...
    - Dream Career: {user_profile.dream_career}
...
def get_chapter_content(chapter_name, user_context):
    ...
    INSTRUCTIONS:
    1. Start with a vivid introduction.
    2. Provide a "Concept Breakdown".
    3. MANDATORY: Provide at least 2 "Master Solved Examples" with step-by-step logic.
    4. Use {interests} analogies.
...
def get_topic_explanation(chapter_name, topic_name, user_context):
    if not client: return {"error": "API Key Missing"}
    ...
    2. **How it works**: Deep conceptual logic.
    3. **Solved Example**: Provide 1-2 actual math problems solved step-by-step.
    4. **Pro-Tip**: A shortcut or common mistake to avoid.
...
def get_ai_doubt_solver(prompt, subject, user_context):
